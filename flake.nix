      ...
      overlay = final: prev:
        ...
        let
          ...
          pluginSourceDirs = builtins.removeAttrs (lib.mapAttrs'
            ...
            (builtins.readDir pluginsDir)) [ "default" "hls-stylish-haskell-plugin" "hls-hlint-plugin" ];
          ...
          tweaks = hself: hsuper:
            with haskell.lib; {
              ...
              hiedb = hself.callHackageDirect { pkg = "hiedb"; ver = "0.4.1.0"; sha256 = "sha256-8TYsMmcRItEQhUp6cKWxt8PFIpTj+jeH+4M506ajR4c="; } {};
      ...
      let
        ...
        mkExe = hpkgs:
          ...
          (enableSharedExecutables (overrideCabal hpkgs.haskell-language-server
            (_: {
              configureFlags = hpkgs.haskell-language-server.configureFlags ++ ["--flags=-hlint"];
              postInstall = ''
